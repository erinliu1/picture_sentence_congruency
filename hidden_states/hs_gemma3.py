# For every stimulus, feed to each Gemma 3 model, pull out the model's internal hidden states at the last word of the sentence, for every layer.
...
def extract_hidden_states(model_id):
    if model_id in GEMMA3_DENSE_LOOKUP:
        model_name = GEMMA3_DENSE_LOOKUP[model_id]
