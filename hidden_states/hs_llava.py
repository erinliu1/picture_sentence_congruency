# For every stimulus, feed to each LLaVA-OneVision model, pull out the model's internal hidden states at the last word of the sentence, for every layer.
...
# All LLaVA-OneVision checkpoints (Qwen2 LLM backbone) are dense, run on a single GPU, and tokenize rating digits "1"-"5" as single tokens.
LLAVA_DENSE_LOOKUP = {
    "llava_ov_05b": "llava-hf/llava-onevision-qwen2-0.5b-ov-hf",
    "llava_ov_7b": "llava-hf/llava-onevision-qwen2-7b-ov-hf",
    "llava_ov_72b": "llava-hf/llava-onevision-qwen2-72b-ov-hf",
}

def extract_hidden_states(model_id):
    if model_id in LLAVA_DENSE_LOOKUP:
        model_class = LlavaOnevisionForConditionalGeneration
        model_name = LLAVA_DENSE_LOOKUP[model_id]
